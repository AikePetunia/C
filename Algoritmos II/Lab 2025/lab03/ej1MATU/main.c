...
int main(int argc, char *argv[]) {
    ...
    int max_temp_anual[YEARS];

    int mes_maxprecip_anual[YEARS];

    printf("Menor temperatura mínima histórica: %g\n",temp_min(table)/10.0);

    printf("Las temperaturas máximas de cada año fueron:\n[ ");
    temp_max_anual(table, max_temp_anual);
    for (int i = 0; i < YEARS; i++) {
        printf("%g ",max_temp_anual[i]/10.0);
    }
    printf("].\n");

    printf("Los meses con más precipitaciones de cada año son:\n[ ");
    mes_precip_max_anual(table, mes_maxprecip_anual);
    for (int i = 0; i < YEARS; i++) {
        printf("%d ",mes_maxprecip_anual[i]);
    }
    printf("].\n");
